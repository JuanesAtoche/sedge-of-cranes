...
> **Build status: Phase 4 — UX polish (v0.4.0).** All five UI states are
> finished, the AR libraries lazy-load after the landing screen, and errors
> fail gracefully. ⚠️ The narration voice is still the ROBOTIC PLACEHOLDER —
> replace it before showing this to children (see Phase 3 section).
...
## Phase 4 notes (v0.4.0)

**Lazy loading.** The landing page is now ~50 KB; A-Frame + MindAR (~1.5 MB)
and the narration only start downloading when the user taps **Start**
(spec §8: cold start ≤ 30 s on 4G; assets lazy-loaded after landing).

**Hardened errors.**
- Missing `targets.mind` or a CDN failure now shows a "Something didn't
  load" screen (not a wrong camera-permission message).
- Permission-denied instructions are tailored per browser (iOS Safari vs
  Android Chrome).
- If narration audio fails, the flock still flies and the user is told.
- WhatsApp/Instagram/Facebook **in-app browsers** get a warning on the
  landing page: open in Safari/Chrome (in-app webviews often block the
  camera — we hit this ourselves in Phase 3 testing).
- A **screen wake lock** keeps the phone awake during the story.

**Unsupported-device fallback.** Old browsers (and load failures) show
`assets/fallback.gif` — a pre-rendered loop of the experience generated by
`tools/render_fallback.py` from the same geometry as the live app.

**Cache busting.** Every deploy bumps `APP_VERSION` in `js/app.js` and the
`?v=` numbers in `index.html`. The landing page shows the version at the
bottom — always confirm the phone displays the version you just deployed.

### Phase 4 test — the five states

| State | How to reach it | What you should see |
|---|---|---|
| 1. Landing | Open the URL | Title, Start, privacy note, mat PDF link, **v0.4.0** at bottom |
| 2. Denied | Tap Start, refuse camera | Instructions specific to your browser + Try again |
| 3. Scanning | Tap Start, allow camera | "Getting the magic ready…" then "Point your camera at the story mat" |
| 4. Experience | Point at the mat | Flock + story (Phase 3 tests all still pass) |
| 5. Unsupported | Open in a very old browser — or temporarily rename `targets.mind` in the repo | Friendly message + looping GIF of the experience |

Also verify: the screen does not auto-lock during the 60 s story, and
losing tracking mid-story shows "Point back at the mat to continue".
...
- [x] **Phase 4 — UX polish:** all five UI states, fallbacks
